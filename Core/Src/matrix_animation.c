...
uint8_t temp_matrix_buffer_head[8] = {0,0,0,0,0,0,0,0};
uint8_t temp_matrix_buffer_tail[8] = {0,0,0,0,0,0,0,0};


void ledAnimation() {
	for (uint8_t i = 0; i < 8; i++) {
		temp_matrix_buffer_head[i] = ((matrix_buffer[i] / 128) == 0)?
				(temp_matrix_buffer_head[i] * 2): (temp_matrix_buffer_head[i] * 2) + 1;
		matrix_buffer[i] = (temp_matrix_buffer_tail[i] / 128 == 0)?
				(matrix_buffer[i] * 2): (matrix_buffer[i] * 2) + 1;
		temp_matrix_buffer_tail[i] = temp_matrix_buffer_head[i];
	}
}
